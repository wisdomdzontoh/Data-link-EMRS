...
$(document).ready(function(){
	...
	function add_dynamic_input_field(count)
	{
		...
		output += '<td><input type="text" name="readings[]" placeholder="Add Programming Languages" class="form-control name_list" /></td>';
	...
	$('#add_name').on('submit', function(event){
		...
		{
			alert("Please Enter at least one readings");
		}
	});
	...
	$(document).on('click', '.edit', function(){
		...
		$.ajax({
			...
			{
				$('#name').val(data.name);
				$('#dynamic_field').html(data.readings);
				$('#action').val('edit');
